  ...
  return (
    ...
    >
      {async (deps: any) => {
        ...
        // `gh pr list --head owner:branch` unreliably matches cross-fork heads (it
        // returned [] while an open PR existed, so create collided with "a pull
        // request ... already exists" and failed the run twice). List by branch
        // name and filter on the owner ourselves.
        const findPr = () =>
          (
            JSON.parse(
              sh("gh", [
                ...
                "open",
                "--json",
                "number,isDraft,url,headRefName,headRepositoryOwner",
                "--limit",
                "100",
              ]) || "[]",
            ) as Array<{ number: number; isDraft: boolean; url: string; headRefName: string; headRepositoryOwner?: { login?: string } }>
          ).find((p) => p.headRefName === input.featureBranch && p.headRepositoryOwner?.login === login);
        let pr = findPr();
        if (!pr) {
